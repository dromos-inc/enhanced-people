...
from .const import CONF_CATEGORY, CONF_DEVICE_TRACKER, CONF_PERSON, DOMAIN
...
async def create_enhanced_people_trackers(
    ...
    person_name = (
        person_state.name
        if person_state and hasattr(person_state, "name")
        else person_entity
    )
...
class EnhancedPersonTracker(TrackerEntity):
    def __init__(
        self, source_entity: str, person_name: str, category: str, entry_id: str
    ):
