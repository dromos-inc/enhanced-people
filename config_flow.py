...
class EnhancedPeopleConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
        ...
        if user_input is not None:
            ...
            if tracker_entry and tracker_entry.device_id:
                ...
                wifi_sensors = [
                    ...
                    if (e.device_id == tracker_entry.device_id and e.domain == "sensor")
                ]
                ...
                for entity in wifi_sensors:
                    ...
                    if (
                        ("_ssid" in entity_name or "wifi_connection" in entity_name)
                        and "bssid" not in entity_name
                    ) or (
                        ("_ssid" in entity_id or "wifi_connection" in entity_id)
                        and "bssid" not in entity_id
                    ):
                        priority_matches.append(entity.entity_id)
                        _LOGGER = logging.getLogger(__name__)
                        _LOGGER.debug(
                            "Found priority WiFi sensor: %s", entity.entity_id
                        )
        ...
        if tracker_entry and tracker_entry.device_id:
            wifi_options = [
                ...
                if (
                    ...
                    and e.domain == "sensor"
                    and (
                        "wifi" in (e.original_name or "").lower()
                        or "ssid" in (e.original_name or "").lower()
                    )
                )
            ]
    ...
    def _create_entry(self):
        ...
        return self.async_create_entry(
            title=title, data=self._user_input, options=options
        )
